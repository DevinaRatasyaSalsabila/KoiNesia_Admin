<!-- Modal -->
<div class="modal fade" id="edit_pengeluaran_{{ $item->id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form class="row g-3 needs-validation" action="{{ route('pengeluaran.update', $item->id) }}"
                id="formEditPengeluaran_{{ $item->id }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="col-md-12 px-3 mt-2">
                        ...
                        <input type="date" class="form-control" id="bsValidation3" name="tanggal"
                            value="{{ $item->tanggal }}" placeholder="Tanggal" required>
                    ...
                    <div class="col-md-12 px-3 mt-2">
                        ...
                        <input type="text" class="form-control" id="bsValidation3" name="nama_pengeluaran"
                            value="{{ $item->nama_pengeluaran }}" placeholder="Nama" required>
                        <div class="invalid-feedback">
                    ...
                    <div class="col-md-12 px-3 mt-2">
                        ...
                        <input type="text" class="form-control nominal-format" name="nominal"
                    ...
                    <div class="col-md-12 px-3 mt-2">
                        ...
                        <input type="text" class="form-control" value="{{ $item->keterangan }}" name="keterangan"
                            id="bsValidation5" placeholder=" Masukkan Keterangan" required>
                        <div class="invalid-feedback">
                            Masukkan Keterangan
                        </div>
                    </div>
...
@push('scripts')
    <script>
        ...
        function formatUang(subject) {
            if (!subject) return "";
            subject = subject.toString().replace(/[^,\d]/g, "");
            let rupiah = subject.replace(/\B(?=(\d{3})+(?!\d))/g, ".");
            return "Rp" + rupiah;
        }

        $(document).on('input', '.nominal-format', function () {
            let value = $(this).val();
            $(this).val(formatUang(value));
        });

        $(document).on('submit', '.needs-validation', function () {
            $(this).find('.nominal-format').each(function () {
                let value = $(this).val();
                let angkaMurni = value.replace(/[^0-9]/g, "");
                console.log("Nominal sebelum kirim:", value, " → setelah clean:", angkaMurni);
                $(this).val(angkaMurni);
            });
        });

        $(document).on('shown.bs.modal', function (e) {
            $(e.target).find('.nominal-format').each(function () {
                let value = $(this).val();
                $(this).val(formatUang(value));
            });
        });
